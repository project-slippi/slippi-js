...
it("should correctly return game settings", () => {
  ...
  expect(settings.players[0]?.characterId).toBe(0x13);
  expect(settings.players[settings.players.length - 1]?.characterId).toBe(0xe);
...
it("should correctly return stats", () => {
  ...
  expect(stats.stocks[stats.stocks.length - 1]?.endFrame).toBe(3694);
  ...
  expect(firstConversion.moves[0]?.moveId).toBe(15);
  expect(firstConversion.moves[firstConversion.moves.length - 1]?.moveId).toBe(17);
...
it("should support realtime parsing", () => {
  ...
  copyBuf(0xe8 * 3);
  data = getData();
  expect(Object.keys(data.frames).length).toBe(3);
  ...
  copyBuf(0x8271b);
  data = getData();
  expect(Object.keys(data.frames).length).toBe(2306);
